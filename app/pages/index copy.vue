<template>
	<v-container>
		...
		<p>There are {{ launches?.length || 0 }} ships.</p>
		<v-table>
			<thead>
				<tr>
			...
			<tbody>
				<tr v-for="launch in launches" :key="launch.mission_name">
					<td class="text-no-wrap">{{ launch.mission_name }}</td>
					<td class="text-no-wrap">{{ launch.launch_date_utc }}</td>
					<td>{{ launch.launch_site }}</td>
					<td class="text-no-wrap">{{ launch.rocket.rocket_name }}</td>
